class Business_card(object):
	#Defines the Business Card class. id: intiger id of card for shuffling purposes, possesion: if a player posses this card, set int to player number, special: special card ability
	def __init__(self, id, name, cost, income, consumer_type, pr_problem, special, possesion):
		self.name = name
		self.cost = cost
		self.income = income
		self.cosumer_type = consumer_type
...
natural_beauty_rhinoplasty = Business_card(2, "NATURAL BEAUTY RHINOPLASTY", 8, 6, ["Luxery", "Health"], [], [], 0)
...
big_journey_suvs = Business_card(12, "BIG JOURNEY SUVS", 12, 8, ["Transportation", "Luxery"], ["Polluting"], [], 0)
...
naive_bottled_water = Business_card(20, "NAIVE BOTTLED WATER", 8, 6, ["Luxery", "Food"], ["Polluting"], [], 0)
